...
public class HelperTypeDAO {
    ...
    public ArrayList getList(Connection conn){
        ...
        Statement st = null;
        ResultSet rs = null; 
        ArrayList<HelperType> helperTypeList = new ArrayList<>();
        try {
            statement = conn.prepareCall(sql);
            statement.setNull(1, Types.NUMERIC);
            statement.registerOutParameter(2, Types.REF_CURSOR);
            ...
            while (rs.next()) {
                ...
                helper.setIdHelperType(rs.getInt(1));
                helper.setNameHelperType(rs.getString(2));
                helperTypeList.add(helper);
            }
            System.out.println("Succesfully listed HelperType");

        } catch (Exception e) {

            JOptionPane.showMessageDialog(null, "Unable to get HelperType list");
            System.out.println(e.getMessage());
        }
        return helperTypeList;
    }
}
